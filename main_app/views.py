...
forest_model = load('main_app/Forest_Model/model.joblib')
...
def Medications(pred):
    file_path = 'main_app/Dataset/medications.csv'
    medications = extract_info_from_csv(file_path,pred)
    list = ast.literal_eval(medications)
    return list

def Description(pred):
    file_path = 'main_app/Dataset/description.csv' 
    description = extract_info_from_csv(file_path,pred)
    return description

def Diets(pred):
    file_path = 'main_app/Dataset/diets.csv' 
    diets = extract_info_from_csv(file_path,pred)
    converted_list = ast.literal_eval(diets)
    return converted_list

def Precautions(keyword):
    with open('main_app/Dataset/precautions_df.csv',mode = 'r',newline = '') as file:
        reader = csv.reader(file)
        for row in reader:
            if row[1] == keyword:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for i in range(1, 133):
                checkbox_name = symptoms[i-1]
                checkbox_value = 1 if form.cleaned_data.get(checkbox_name, False) else 0
                checkbox_values.append(checkbox_value)
            
            # Do something with checkbox_values, for example:
            # print(checkbox_values)  # This will print the list of checkbox values in your console
            input = np.array(checkbox_values).reshape(1,-1)
            pred = forest_model.predict(input)
            medications = Medications(disease[pred[0]])
            description = Description(disease[pred[0]])
            diets = Diets(disease[pred[0]])
            precautions = Precautions(disease[pred[0]])
            workout = Workout(disease[pred[0]])
            return render(request,'result.html',{'result':disease[pred[0]],'medications':medications,'description':description,'diets':diets,'precautions':precautions,'workout':workout})
            # You can also pass this list to a template if needed:
            # return render(request, 'main.html', {'form': form, 'checkbox_values': checkbox_values})
    else:
        form = CheckboxForm()
...
def Temp(request):
    template = loader.get_template('app_base.html')
    return HttpResponse(template.render())